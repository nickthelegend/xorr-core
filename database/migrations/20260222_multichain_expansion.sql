...
ALTER TABLE public.pools ADD COLUMN IF NOT EXISTS physical_balance numeric default 0;
ALTER TABLE public.pools ADD COLUMN IF NOT EXISTS lp_balance numeric default 0;
ALTER TABLE public.pools ADD COLUMN IF NOT EXISTS network_name text;

-- Add unique constraint for cross-chain identification
ALTER TABLE public.pools DROP CONSTRAINT IF EXISTS pools_name_chain_id_key;
ALTER TABLE public.pools ADD CONSTRAINT pools_name_chain_id_key UNIQUE (name, chain_id);

-- 2. Clear existing demo data to avoid duplication (Optional, but recommended for clean state)
-- DELETE FROM public.pools;

-- 3. Insert/Update Vaults for SEPOLIA (11155111)
...
-- 4. Insert Vaults for AVALANCHE FUJI (43113)
INSERT INTO public.pools (name, token_address, chain_id, network_name, apr, tvl)
VALUES 
...
-- 5. Insert Vaults for BASE SEPOLIA (84532)
INSERT INTO public.pools (name, token_address, chain_id, network_name, apr, tvl)
VALUES 
...
-- 6. Insert Vaults for CRONOS TESTNET (338)
INSERT INTO public.pools (name, token_address, chain_id, network_name, apr, tvl)
VALUES 
